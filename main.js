...
const factor =
(this.exponent * rNormExp) / effectiveBoundary / denomSq;
const forceMag =
this.strength * Math.min(factor, this.maxMultiplier / r);
...
textureLoader.load("medres/eso0932a.JPG", (texture) => {
...
initDomElements() {
const ids = ["timeDisplay", "cageEnergy", "nearestBodyStatus"];
for (const id of ids) {
...
const baseColor = new THREE.Color(this.trailColors[colorIndex]);
...
(layer.maxRadius - layer.minRadius);
...
velocities.push({
vx: (Math.random() - 0.5) * 0.015 * layer.turbulence,
vy: (Math.random() - 0.5) * 0.015 * layer.turbulence,
vz: (Math.random() - 0.5) * 0.015 * layer.turbulence,
noiseOffsetX: Math.random() * 1000,
noiseOffsetY: Math.random() * 1000,
noiseOffsetZ: Math.random() * 1000,
turbulenceScale: 0.3 + Math.random() * 0.7,
});
}
...
const velocities = dustLayer.velocities;
...
const noiseX =
Math.sin(this.dustTime * 0.3 + vel.noiseOffsetX) *
vel.turbulenceScale;
const noiseY =
Math.cos(this.dustTime * 0.25 + vel.noiseOffsetY) *
vel.turbulenceScale;
const noiseZ =
Math.sin(this.dustTime * 0.35 + vel.noiseOffsetZ) *
vel.turbulenceScale;

positions[idx] += vel.vx + noiseX * 0.005;
positions[idx + 1] += vel.vy + noiseY * 0.005;
positions[idx + 2] += vel.vz + noiseZ * 0.005;
...
config.targetY =
-normalizedY * config.maxOffsetY * config.sensitivity;
}
...
for (let i = 0; i < this.bodies.length; i++) {
const body = this.bodies[i];
const bodyPos = new THREE.Vector3(body.x, body.y, body.z);
const dist = raycaster.ray.distanceToPoint(bodyPos);

const bodyToCamera = this.camera.position.distanceTo(bodyPos);
const pixelSize =
(2 * bodyToCamera * tanHalfFov) / window.innerHeight;
const effectiveRadius = Math.max(
body.r,
minScreenRadius * pixelSize,
);
...
const com = this.computeCenterOfMass();
const targetPosition = new THREE.Vector3(com.x, com.y, com.z);
...
config.currentX +=
(config.targetX - config.currentX) * config.smoothing;
config.currentY +=
(config.targetY - config.currentY) * config.smoothing;
...
const damping = this.gravityCage.computeDamping(
body,
this.displayScale,
);
accelerations.push({
...
for (
let stepIndex = 0;
stepIndex < yoshidaCoeffs.length;
stepIndex++
) {
...
const rotateAmount =
(rotateConfig.speed * deltaTime * Math.PI) / 180;
...
if (
rotateConfig.minAngle !== null &&
rotateConfig.maxAngle !== null
) {
...
const dist = this.gravityCage.computeDistance(
body,
this.displayScale,
);
const boundaryR =
this.gravityCage.boundaryRadius / this.displayScale;
...
if (rotateAngleEl) {
const currentDeg =
((((this.autoRotate.currentAngle * 180) / Math.PI) % 360) + 360) %
360;
...
"linear-gradient(135deg, #2a5a2a 0%, #1a4a1a 100%)";
...
document.getElementById("autoZoomSensitivity").value =
defaultSensitivity;
...
document.getElementById("autoRotateSpeedRange").value =
sim.autoRotate.speed;
...
document.getElementById("mouseFollowMaxX").value =
...
document.getElementById("mouseFollowMaxY").value =
...
document
.getElementById("btnCageShowBoundaries")
...
document.getElementById("cageBoundaryRange").value =
...
document.getElementById("cageWarningRange").value =
...
document.getElementById("cageExponentVal").textContent =
defaults.exponent;
document.getElementById("cageMaxMult").value = defaults.maxMultiplier;
document.getElementById("cageMaxMultRange").value =
...
document.getElementById("cageDampingRange").value =
...
const dustDriftSpeedRange = document.getElementById(
"dustDriftSpeedRange",
);
...
const dustLightIntensityInput =
document.getElementById("dustLightIntensity");
const dustLightIntensityRange = document.getElementById(
"dustLightIntensityRange",
);
const dustLightIntensityVal = document.getElementById(
"dustLightIntensityVal",
);
...
const dustMaxViewDistanceInput = document.getElementById(
"dustMaxViewDistance",
);
...
if (zoomEl)
zoomEl.textContent = sim.camera.position.length().toFixed(0);
}, 100);